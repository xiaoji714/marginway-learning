...
- Vitest 发现所有 `tests/*.spec.ts`；使用真实 SQLite/CLI/Native Messaging，mock 限在外部边界。
- packages 与 apps 运行源码逐文件 statements/branches/functions/lines 四项100%；覆盖率扫描阻止漏文件。Chrome 端到端内嵌视觉检查，范围和边界见[测试契约](testing.md)。
