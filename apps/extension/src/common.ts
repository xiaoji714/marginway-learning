...
`;
  ...
  function discussionCard({
    ...
    const explanation = el("div", null, "lc-explanation");
    ...
    const collect = icon(
      button("收藏单词", () =>
        busy(collect, async () => {
          ...
          result.textContent = `已收藏“${selectedWord}”，原文语境已关联`;
        }),
      ),
      "bookmark-simple",
    );
    ...
    source.append(el("span", resource.title, "lc-source-title"));
    ...
    collect.disabled = !selectionText;
    collect.textContent =
      selectionText && !/\s/.test(selectionText) ? "收藏单词" : "收藏选中内容";
    icon(collect, "bookmark-simple");
    highlight();
